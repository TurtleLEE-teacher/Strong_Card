...
import { dateTimeShort, monthShort, won, wonShort } from '@/lib/format';
import { byRecentFirst, nextMonthKey, previousMonthKey, toMonthKey, type MonthKey } from '@/lib/date';
import { cardHref, dashboardHref } from '@/lib/routes';
import type { CardId, PerformanceVerdict } from '@/lib/types';

/**
 * 카드 상세 본문.
 *
 * `/card/[cardId]`(이번 달)와 `/card/[cardId]/[month]`(지난달)가 이걸 함께
 * 쓴다. 대시보드와 마찬가지로 두 라우트가 각자 그리면 언젠가 한쪽에만
 * 기능이 빠진다.
 */
export async function CardDetailView({
  cardId,
  ...
  const { month, snapshots, transactions, daysRemaining, fetchedAt } =
    await getDashboardData(requestedMonth);
  ...
  /**
   * 이 룰의 할인을 받으면 결제액이 실적에서 통째로 빠지는가.
   *
   * 탄탄대로 Trendy가 그렇다. 화면 어디서도 이걸 말하지 않으면 사용자는
   * "20% 할인을 받았는데 왜 실적이 안 늘었지"를 영영 못 푼다.
   */
  const excludesSpend = new Set(
    card.benefits.filter((rule) => excludesSpendFromPerformance(card, rule)).map((r) => r.id),
  );
      ...
      <header className="mb-6 flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
            {card.last4.join(', ')} · 연회비 {won(card.annualFee)}
          </p>
        </div>
      </header>

      {/* 달 이동은 카드 이름 아래에 둔다 — 어느 카드의 몇 월을 보고 있는지가
          한 덩어리로 읽혀야 한다. */}
      <div className="mb-4">
        <MonthNav month={month} href={(m) => cardHref(card.id, m)} />
      </div>
        ...
        {card.performance.required && snapshot.nextTier && (
          <div className="mt-4">
            <Meter
              ...
              ariaLabel={`${monthShort(month)} 실적 진행률`}
            />
          </div>
        )}

        {/*
          "혜택을 받으면 실적에서 빠지는" 카드라는 사실을 실적 옆에 못 박는다.
          제외 내역 패널까지 내려가야 알 수 있으면, 정작 이 함정이 문제가 되는
          순간(구간을 못 넘긴 채 월말)에 아무도 안 읽는다.
        */}
        {excludesSpend.size > 0 && (
          <p className="mt-3 text-[11px] leading-relaxed" style={{ color: 'var(--status-serious)' }}>
            ⚠ 이 카드는{' '}
            <strong>
              {card.benefits
                .filter((r) => excludesSpend.has(r.id))
                .map((r) => r.label)
                .join(' · ')}
            </strong>{' '}
            할인을 받은 결제는 <strong>결제액 전체가 실적에서 빠집니다.</strong> 할인은 그대로
            받습니다 — 빠지는 건 실적뿐입니다.
          </p>
        )}
      </Panel>
            ...
            {earned.map(({ benefit, tx }) => (
              <li key={benefit.transactionId} className="flex items-center gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  ...
                  {/* 이 할인을 받은 대가로 결제액이 실적에서 빠진 건.
                      혜택 금액 옆이 이 사실을 말할 수 있는 유일한 자리다. */}
                  {excludesSpend.has(benefit.ruleId) && benefit.netAmount > 0 && (
                    <p className="mt-0.5 text-[11px]" style={{ color: 'var(--status-serious)' }}>
                      실적에서 {won(Math.abs(tx.krwAmount))} 빠짐 (혜택받은 결제)
                    </p>
                  )}
                </div>
        ...
        {monthTx.length === 0 ? (
          ...
          <ul className="divide-y" style={{ borderColor: 'var(--border)' }}>
            {monthTx.map((tx) => {
              ...
              const verdict = snapshot.performanceVerdicts[tx.id] as
                | PerformanceVerdict
                | undefined;
              return (
                <li key={tx.id} className="flex items-center gap-3 py-2.5">
                  ...
                  <div className="shrink-0 text-right">
                    ...
                    {/*
                      ...
                      실적 숫자는 카드 위쪽에 합계로만 있어서, 어떤 결제가
                      거기 안 들어갔는지는 이 목록에서 볼 방법이 없었다.
                      특히 '혜택받은 결제'는 화면상 혜택을 받아 좋아 보이는
                      바로 그 줄이 실적을 깎은 줄이라, 여기 안 적으면
                      영영 연결되지 않는다.
                    */}
                    {verdict && verdict !== '인정' && (
                      <p className="text-[11px]" style={{ color: 'var(--status-serious)' }}>
                        실적 제외 · {VERDICT_LABELS[verdict]}
                      </p>
                    )}
