...
export const KB_TANTANDAERO: Card = {
  ...
  benefits: [
    ...
    // 이 두 영역의 할인을 받은 거래는 실적에서 빠진다.
    {
      ...
      type: 'discount',
      rate: 0.2,
      // 약관은 **업종**(미용원·피부미용원·화장품점…) 기준인데 앱은 업종
      // 코드를 못 본다. 그래서 세 겹으로 잡는다.
      //   1) 브랜드 사전 — 이름에 '헤어'가 없는 프랜차이즈('차홍룸')
      //   2) 이름 키워드 — 간판에 업종이 드러나는 곳
      //   3) 노션 카테고리 '미용' — 둘 다 아닌 동네 미용실은 사람이 적는다
      // 2026-09 차홍룸 30만원이 셋 중 어디에도 안 걸려 20,000원을 놓쳤다.
      match: {
        brands: [...BRAND_GROUPS.BEAUTY_SALON],
        categories: ['미용'],
        keywords: [
          // 미용/화장품: 미용원, 피부미용원, 화장품점
          '미용실', '미용원', '미용', '헤어', '살롱', 'SALON', '뷰티', 'BEAUTY',
          '이용원', '바버', 'BARBER', '네일', '피부관리', '에스테틱', '왁싱', '화장품',
    ...
    // ── Daily 서비스 (영역별 10,000 / 15,000) ──────────────────────────
    {
      ...
      match: {
        ...
        categories: ['카페'],
        // 약관의 베이커리는 **제과점 업종**이다. '(주)한스케익 아브뉴' 21,900원에
        // KB가 10%를 실제로 줬다 (사용자 확인, 2026-09) — 이름에 '베이커리'가
        // 없어도 케익·제과가 들어가면 제과점이다.
        keywords: [
          '베이커리', '케익', '케이크', '제과', '파리바게뜨', '뚜레쥬르',
          '배스킨라빈스', '설빙', '아이스크림',
        ],
      },
