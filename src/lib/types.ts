 ...
 *
 * '미용'은 앱이 추가한 값이다. 미용 혜택은 업종 기준인데 브랜드 사전으로는
 * 동네 미용실을 영영 못 잡는다 — 노션에서 이 값을 고르면 걸린다.
 */
export type TxCategory =
  | '식비' | '카페' | '교통' | '생활' | '의료'
  | '교육' | '여가' | '구독' | '업무' | '금융' | '미용' | '기타';

/** TxCategory 전체 목록. 노션에서 읽은 문자열이 유효한 값인지 가릴 때 쓴다. */
export const TX_CATEGORIES: readonly TxCategory[] = [
  '식비', '카페', '교통', '생활', '의료', '교육', '여가', '구독', '업무', '금융', '미용', '기타',
];
...
export interface Transaction {
  ...
  /**
   * 노션 `브랜드` 열에서 **사람이 지정한** 브랜드 키.
   *
   * '판교서일주유소'처럼 상호에 브랜드가 없는 가맹점은 이름을 아무리 뜯어봐도
   * GS인지 S-OIL인지 알 수 없다. 거기 가 본 사람만 아는 사실이라 노션에서
   * 직접 지정하게 하고, 그 값을 이름 사전보다 **우선**해서 쓴다.
   *
   * 한 번 지정하면 같은 가맹점명의 다른 달 거래에도 따라붙는다
   * (fetchMerchantBrandMap).
   */
  brand?: string | null;
  /**
   * 노션 `브랜드` 열의 원문.
   *
   * 사전에 없는 값을 적었을 때(예: '쉘') `brand`는 null이 되는데, 원문까지
   * 버리면 화면이 "지정했는데 왜 아무 일도 안 일어나지"에 답할 수 없다.
   */
